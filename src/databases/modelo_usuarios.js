...
export async function usuariosConFiltro(activo, ordenar, desc, limit, offset) {
    let query = 'SELECT usuario_id, nombre, apellido, nombre_usuario, tipo_usuario, celular, foto FROM usuarios WHERE activo = ?';
    const params = [activo];
...
export async function usuariosPorId(id){
    const [rows] = await pool.query('SELECT usuario_id, nombre, apellido, nombre_usuario, tipo_usuario, celular, foto FROM usuarios WHERE usuario_id = ?', [id]);
    return rows[0];
}
...
export async function actualizarUsuario(id, datos){
    const {nombre, apellido, nombre_usuario, tipo_usuario, celular } = datos;
        await pool.query(`UPDATE usuarios SET nombre = ?, apellido = ?, nombre_usuario = ?, tipo_usuario = ?, celular = ? WHERE usuario_id = ?`,
        [nombre, apellido, nombre_usuario, tipo_usuario, celular, id]);
    return datos;
}
